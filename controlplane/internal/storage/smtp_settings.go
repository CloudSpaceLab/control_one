...
func (s *Store) GetSMTPSettings(ctx context.Context, tenantID uuid.UUID) (*SMTPSettings, error) {
 ...
 password_ciphertext, password_nonce, sender_name, sender_email, enabled, updated_at
 FROM smtp_settings WHERE tenant_id=$1`, tenantID).Scan(&c.TenantID, &c.Host, &c.Port, &c.TLSMode,
		&c.AuthEnabled, &c.Username, &c.PasswordCiphertext, &c.PasswordNonce, &c.SenderName, &c.SenderEmail, &c.Enabled, &c.UpdatedAt)
...
func (s *Store) UpsertSMTPSettings(ctx context.Context, c SMTPSettings, replacePassword bool) error {
 ...
 (tenant_id,host,port,tls_mode,auth_enabled,username,password_ciphertext,password_nonce,sender_name,sender_email,enabled)
 ...
 password_ciphertext=CASE WHEN $12 THEN EXCLUDED.password_ciphertext ELSE smtp_settings.password_ciphertext END,
 password_nonce=CASE WHEN $12 THEN EXCLUDED.password_nonce ELSE smtp_settings.password_nonce END,
 sender_name=EXCLUDED.sender_name,sender_email=EXCLUDED.sender_email,enabled=EXCLUDED.enabled,updated_at=NOW()`,
		c.TenantID, c.Host, c.Port, c.TLSMode, c.AuthEnabled, c.Username, c.PasswordCiphertext, c.PasswordNonce, c.SenderName, c.SenderEmail, c.Enabled, replacePassword)
	return err
}
