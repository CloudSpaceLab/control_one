...
: "${NETBIRD_DOMAIN:?NETBIRD_DOMAIN is required}"

NETBIRD_INSTALLER_URL="${NETBIRD_INSTALLER_URL:-https://github.com/netbirdio/netbird/releases/latest/download/getting-started.sh}"
NETBIRD_INSTALL_SCRIPT="${NETBIRD_INSTALL_SCRIPT:-./getting-started.sh}"
FETCH_NETBIRD_INSTALLER="${FETCH_NETBIRD_INSTALLER:-false}"
RUN_NETBIRD_INSTALLER="${RUN_NETBIRD_INSTALLER:-false}"
...
NETBIRD_PROVIDER_ACCOUNT_ID="${NETBIRD_PROVIDER_ACCOUNT_ID:-${NETBIRD_DOMAIN}}"
NETBIRD_DISPLAY_NAME="${NETBIRD_DISPLAY_NAME:-NetBird ${NETBIRD_DOMAIN}}"
...
if [[ ! -f "${NETBIRD_INSTALL_SCRIPT}" ]]; then
  if is_true "${FETCH_NETBIRD_INSTALLER}"; then
    require_cmd curl
    echo "Downloading official NetBird installer to ${NETBIRD_INSTALL_SCRIPT}"
    curl -fsSL "${NETBIRD_INSTALLER_URL}" -o "${NETBIRD_INSTALL_SCRIPT}"
    chmod 0755 "${NETBIRD_INSTALL_SCRIPT}"
  else
    echo "NetBird installer not found at ${NETBIRD_INSTALL_SCRIPT}" >&2
    echo "Set FETCH_NETBIRD_INSTALLER=true to stage it, or pre-place the reviewed script." >&2
    exit 1
  fi
fi

if is_true "${RUN_NETBIRD_INSTALLER}"; then
  echo "Running reviewed NetBird installer for ${NETBIRD_DOMAIN}"
  export NETBIRD_DOMAIN
  bash "${NETBIRD_INSTALL_SCRIPT}"
else
  echo "NetBird installer staged at ${NETBIRD_INSTALL_SCRIPT}; set RUN_NETBIRD_INSTALLER=true to execute it."
fi
...
jq -n \
  --arg provider "netbird" \
  --arg account_id "${NETBIRD_PROVIDER_ACCOUNT_ID}" \
  --arg display_name "${NETBIRD_DISPLAY_NAME}" \
  --arg endpoint_url "https://${NETBIRD_DOMAIN}" \
...
if is_true "${APPLY_CONTROL_ONE_PROVIDER}"; then
  ...
  echo
  echo "Applied Control One NetBird provider account."
fi
