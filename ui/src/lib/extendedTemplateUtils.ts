...
export function getTemplateIcon(template: ExtendedTemplate): string {
...
export function getTemplateTypeDescription(type: TemplateType): string {
  switch (type) {
    case TemplateType.JOB:
...
export function getTemplateStatus(template: ExtendedTemplate): string {
...
export function summarizeTemplates(templates: ExtendedTemplate[]): TemplateSummary {
...
export function filterTemplates(templates: ExtendedTemplate[], filter: TemplateFilter): ExtendedTemplate[] {
  return templates.filter(template => {
    ...
    if (filter.labels) {
      ...
      const hasAllLabels = Object.entries(filter.labels).every(([key, value]) => 
        templateLabels.some(([tKey, tValue]) => tKey === key && tValue === value)
      );
      if (!hasAllLabels) {
        return false;
      }
...
export function validateTemplateParameters(
  ...
  switch (template.type) {
    case TemplateType.CONFIG:
      const configTemplate = template as ConfigTemplate;
    ...
    case TemplateType.JOB:
      const jobTemplate = template as JobTemplate;
...
export function getDefaultParameters(template: ExtendedTemplate): Record<string, unknown> {
  switch (template.type) {
    case TemplateType.CONFIG:
      const configTemplate = template as ConfigTemplate;
...
export function getTemplateProviders(templates: ExtendedTemplate[]): string[] {
  return [...new Set(templates.map(t => t.provider))].sort();
}

export function getTemplateLabels(templates: ExtendedTemplate[]): Record<string, string[]> {
...
export function isJobTemplate(template: ExtendedTemplate): template is JobTemplate {
  return template.type === TemplateType.JOB;
}

export function isConfigTemplate(template: ExtendedTemplate): template is ConfigTemplate {
  return template.type === TemplateType.CONFIG;
}

export function isComplianceTemplate(template: ExtendedTemplate): template is ComplianceTemplate {
