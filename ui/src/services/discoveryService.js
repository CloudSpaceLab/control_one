class DiscoveryService {
     ...
     */
    async discoverNodes(ranges, options = {}) {
        const startTime = Date.now();
        const { ports = [22, 80, 443, 8080, 8443], // Common ports
        timeout = 2000, maxConcurrent = 20 } = options;
        const allNodes = [];
        const scannedRanges = [];
        let totalScanned = 0;
        let onlineCount = 0;
        ...
        for (let i = 0; i < ipsToScan.length; i += maxConcurrent) {
            ...
            const batchPromises = batch.map(ip => this.scanIP(ip, ports, timeout));
            try {
     ...
     */
    getFirstIP(network, prefixLength) {
     ...
     */
    getLastIP(network, prefixLength) {
        const parts = network.split('.').map(Number);
        if (prefixLength === 24) {
