...
const sheetVariants = cva(
  'fixed z-[80] gap-4 bg-surface p-6 shadow-[var(--shadow-panel)] transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300 border-border-subtle',
  {
    variants: {
      side: {
  ...
  ({ side = 'right', className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute right-3 top-3 z-10 grid h-8 w-8 place-items-center rounded-md opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-brand-500 disabled:pointer-events-none">
          <X className="h-4 w-4" />
