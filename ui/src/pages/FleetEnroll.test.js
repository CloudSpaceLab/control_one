...
const startFleetEnroll = vi.fn();
const getFleetEnrollStatus = vi.fn();
const getNode = vi.fn();
const showToast = vi.fn();
vi.mock('../hooks/useApiClient', () => ({
    useApiClient: () => ({
        startFleetEnroll: (payload) => startFleetEnroll(payload),
        getFleetEnrollStatus: (id) => getFleetEnrollStatus(id),
...
describe('FleetEnroll', () => {
    ...
    it('renders the form heading', () => {
        render(_jsx(FleetEnroll, {}));
        expect(screen.getByRole('heading', { name: /bulk enrol hosts/i })).toBeInTheDocument();
    });
    it('rejects submit with no targets', async () => {
        ...
        await waitFor(() => expect(screen.getByRole('alert')).toHaveTextContent(/add at least one host/i));
    ...
    it('submits the fleet enroll request with parsed targets and starts polling', async () => {
        ...
        const payload = startFleetEnroll.mock.calls[0][0];
        ...
        await waitFor(() => expect(screen.getByRole('table', { name: /per-host enrollment progress/i })).toBeInTheDocument());
