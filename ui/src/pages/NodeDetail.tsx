...
function KnowledgeGraphTab({ nodeId }: { nodeId: string }) {
  return (
    <Panel padding="md" eyebrow="KNOWLEDGE GRAPH" title="Services + URLs on this node">
      <Alert variant="info" title="Coming online">
        Service discovery rolls out per agent — once your agent reports listening
        ports and probes localhost services, they appear here. The full
        knowledge_graph.md for this tenant is generated by the controlplane and
        is available at <code className="font-mono text-xs">/api/v1/tenants/&lt;tenant&gt;/knowledge_graph.md#node-{nodeId}</code>.
      </Alert>
    </Panel>
  );
}
