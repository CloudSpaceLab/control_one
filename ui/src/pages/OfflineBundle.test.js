...
function makeApiClientStub() {
    return {
        ...
        buildBundleDownloadUrl: vi.fn((options) => {
...
describe('OfflineBundle', () => {
    ...
    beforeEach(() => {
        ...
        vi.spyOn(useApiClientModule, 'useApiClient').mockReturnValue(api);
    ...
    it('loads the enrollment token list and renders the picker', async () => {
        render(_jsx(OfflineBundle, {}), { wrapper: Wrapper });
        await waitFor(() => {
    ...
    it('triggers the download with the override token when the picker omits it', async () => {
        ...
        await user.click(screen.getByRole('button', { name: /download bundle/i }));
        await waitFor(() => {
            expect(windowLocationAssign).toHaveBeenCalledWith(expect.stringContaining('token=cot_abc'));
        });
    });
    it('copies the SCP command via the clipboard helper', async () => {
        ...
        await waitFor(() => {
            expect(writeTextMock).toHaveBeenCalledWith(expect.stringContaining('controlone-bundle-linux-amd64.tar.gz'));
        });
